...
public class UndoRedoHelper {
    ...
    public void storePersistentState(UndoRedoStore editor, String prefix) {
        // Store hash code of text in the editor so that we can check if the
        // editor contents has changed.
        editor.putString(prefix + ".hash",
                String.valueOf(mTextView.getText().toString().hashCode()));
    ...
    public boolean restorePersistentStateFromStr(UndoRedoStore sp, String prefix)
            throws IllegalStateException {
        boolean ok = doRestorePersistentStateFromStr(sp, prefix);
        if (!ok) {
            mEditHistory.clear();
        }
        return ok;
    ...
    private boolean doRestorePersistentStateFromStr(UndoRedoStore sp, String prefix) {
